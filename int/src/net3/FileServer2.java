  ...
  public static void main(String[] args) throws Exception {
    ...
    try (ServerSocket server = new ServerSocket(30000)) {
      while (true) {
        ...
        System.out.println(
          "### 파일서버와 클라이언트의 스트림이 서로 연결됨... "
        );

        System.out.println(
          "### 파일서버가 클라이언트로부터 요청메세지를 수신함..."
        );
        int command = in.readInt();
        if (net3.Command.REQ_UPLOAD == command) {
          System.out.println(
            "### 파일서버가 클라이언트로부터 REQ_UPLOAD 요청을 받음..."
          );
        } else if (Command.REQ_DOWNLOAD == command) {
          System.out.println(
            "### 파일서버가 클라이언트로부터 REQ_DOWNLOAD 요청을 받음..."
          ); //다운로드요청한 파일명 조회
          String filename = in.readUTF();
          System.out.println(
            "### 파일 클라이언트가 다운로드 요청한 파일명: " + filename
          );
          ...
          System.out.println(
            "### 파일서버가 클라이언트로부터 REQ_Files 요청을 받음..."
          );
        }
      }
